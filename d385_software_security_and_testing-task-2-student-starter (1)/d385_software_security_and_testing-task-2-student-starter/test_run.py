...
Run with:   C3: All tests must execute successfully (pass when run with pytest)
...
def test_general_vuln_1_hardcoded_secrets_remediated():
    ...
    """
    assert app.secret_key != "supersecretkeyforflasksessions"
    print("[PASS] Test 1: Hardcoded secret remediation succeeded; the Flask secret key is not hardcoded.")
#   The print() statement above provides a clear message indicating that the secret key has been successfully changed from the hardcoded value to a more secure alternative, such as an environment variable or a generated token.
...
def test_general_vuln_2_plaintext_passwords_remediated():
    ...
    """
    for username, data in USERS_DB.items():
        assert "password" not in data, f"{username} still has a plaintext password"
        assert data.get("password_hash"), f"{username} is missing a password hash"
    print("[PASS] Test 2: Plaintext password remediation succeeded; user passwords are stored as hashes.")
# The print() statement above confirms that all user passwords have been transitioned from plaintext storage to hashed storage.
...
def test_api_vuln_1_authentication_required():
    ...
    with app.test_client() as client:
        response = client.get("/api/v1/rentals")
    assert response.status_code == 401
    print("[PASS] Test 3: API authentication remediation succeeded; unauthenticated requests receive 401 Unauthorized.")
#   The print() statement above confirms that the API endpoint correctly enforces authentication, returning a 401 Unauthorized status for requests made without valid credentials.
...
def test_api_vuln_2_authorization_enforced():
    ...
    print("[PASS] Test 4: Authorization remediation succeeded; regular users receive 403 Forbidden for admin access.")

#   The print() statement above confirms that the application correctly enforces role-based access control, preventing non-admin users from accessing admin-only endpoints and returning a 403 Forbidden status.
# ============================================================
